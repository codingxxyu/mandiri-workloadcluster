...
- During install, SeedImage uses the generic path `/dev/elemental-install-target`. On each Live ISO, symlink the host OS disk (for example `/dev/sda`) to that path.
