...
> 重要：下面的 YAML 是交付模板。带 `<...>` 的字段不能直接 apply。`MachineInventory.spec.storage` 必须通过匹配 ACP 4.3.2 Provider revision 的 `storagectl` 生成原子补丁，不能手工猜测初始化 approval。
...
export GLOBAL_REGISTRY=<existing-global-registry>:11443
...
所有下面的管理面命令使用现有 Global kubeconfig：
...
```yaml
data:
  v1.34.5: <existing-global-registry>:11443/tkestack/baremetal-base-image:v4.3.2-1-1.34.5-3
```
...
for inventory in \
  ...
  olvm-workloadcluster-f785380f-a4d3-4bb6-a6b1-be5c253d7a62
do
  kubectl --kubeconfig "${GLOBAL_KUBECONFIG}" -n "${BM_NS}" \
...
```bash
export BM_INVENTORY=<one-inventory-name>
...
storage:
  ...
  volumes:
    - name: cpaas-data
      source:
        deviceID: <actual-stable-id-of-100g-data-disk>
        minimumSize: 90Gi
      filesystem:
...
```bash
export BM_INVENTORY=<one-inventory-name>
export BM_STORAGECTL=<path-to-matching-acp-4.3.2-storagectl>
...
```bash
kubectl --kubeconfig "${GLOBAL_KUBECONFIG}" auth can-i update \
...
选择严格更大的整数作为 `<next-counter>`，用相同 Provider revision 的 `storagectl` 生成 patch：
...
```bash
kubectl --kubeconfig "${GLOBAL_KUBECONFIG}" apply \
...
metadata:
  annotations:
    cpaas.io/registry-address: <existing-global-registry>:11443
...
- `<ACP-4.3.2-provider-supported-value>`；
...
```bash
kubectl --kubeconfig "${GLOBAL_KUBECONFIG}" -n cpaas-system get \
...
export WORKLOAD_KUBECONFIG=${PWD}/workload-kubeconfig
...
```bash
kubectl --kubeconfig "${GLOBAL_KUBECONFIG}" -n cpaas-system \
  get machineinventory <inventory-name> -o json \
